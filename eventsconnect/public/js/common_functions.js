...
const generate_graph = (chart_name, element, type = "line") => {
	...
	frappe.call({
		method: "eventsconnect.eventsconnect.utils.get_chart_data",
		args: {
	...
	this.batch_dialog = new frappe.ui.Dialog({
		...
		fields: [
			...
			},
			{
				...
				label: __("Category"),
				fieldname: "category",
				options: "Events Connect Category",
				only_select: 1,
...
const save_batch = (values) => {
	...
	frappe.call({
		method: "eventsconnect.eventsconnect.doctype.eventsconnect_batch.eventsconnect_batch.create_batch",
		args: args,
		callback: (r) => {
			if (r.message) {
				frappe.show_alert({
