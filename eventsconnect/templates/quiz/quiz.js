...
const quiz_summary = (e = undefined) => {
	...
	frappe.call({
		method: "eventsconnect.eventsconnect.doctype.eventsconnect_quiz.eventsconnect_quiz.quiz_summary",
		args: {
			quiz: quiz_name,
...
const is_answer_correct = (type, user_answers, element) => {
	frappe.call({
		async: false,
		method: "eventsconnect.eventsconnect.doctype.eventsconnect_quiz.eventsconnect_quiz.check_answer",
		args: {
...
const add_icon = (element, icon) => {
	...
	$(element).siblings(".option-text").html(`
        <div>
            <img class="d-inline mr-3" src="/assets/eventsconnect/icons/${icon}.svg">
            ${label}
        </div>
    `);
};
