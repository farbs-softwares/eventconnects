...
frappe.ui.form.on("Event Lesson", {
	setup: function (frm) {
		frm.trigger("setup_help");
	},
	setup_help(frm) {
		let quiz_link = `<a href="/app/eventsconnect-quiz"> ${__("Quiz List")} </a>`;
		let exercise_link = `<a href="/app/eventsconnect-exercise"> ${__(
			"Exercise List"
		)} </a>`;
		...
		frm.get_field("help").html(`
            ...
            <table class="table">
                ...
                <tr>
                    ...
                    <td>
                        {{ Quiz("eventsconnect_quiz_id") }}
                    </td>
                    <td>
                        ${__(
							"Copy and paste the syntax in the editor. Replace 'eventsconnect_quiz_id' with the ID of the Quiz you want to add. You can get the ID of the quiz from the {0}.",
							[quiz_link]
						)}
                    </td>
                </tr>
