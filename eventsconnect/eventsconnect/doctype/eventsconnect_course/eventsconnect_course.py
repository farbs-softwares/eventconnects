...
from eventsconnect.eventsconnect.utils import get_chapters, can_create_courses
...
class EventsConnectCourse(Document):
	...
	def after_insert(self):
		capture("course_created", "eventsconnect")
	...
	def send_email_to_interested_users(self):
		...
		args = {
			"title": self.title,
			"course_link": f"/eventsconnect/courses/{self.name}",
		...
		for user in interested_users:
			...
			email_args = frappe._dict(
				...
				template="eventsconnect_course_interest",
				args=args,
				now=True,
			)
			frappe.enqueue(
