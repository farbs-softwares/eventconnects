describe("Event Creation", () => {
	it("creates a new event", () => {
		cy.login();
		cy.wait(1000);
		cy.visit("/eventsconnect/events");

		// Create a event
		cy.get("a").contains("New Event").click();
		cy.wait(1000);
		cy.url().should("include", "/events/new/edit");

		cy.get("label").contains("Title").type("Test Event");
		cy.get("label")
			.contains("Short Introduction")
			.type("Test Event Short Introduction to test the UI");
		cy.get("div[contenteditable=true").invoke(
			"text",
			"Test Event Description. I need a very big description to test the UI. This is a very big description. It contains more than once sentence. Its meant to be this long as this is a UI test. Its unbearably long and I'm not sure why I'm typing this much. I'm just going to keep typing until I feel like its long enough. I think its long enough now. I'm going to stop typing now."
		);
		...
		// View Event
		cy.wait(1000);
		cy.visit("/eventsconnect");
		cy.wait(500);
		cy.url().should("include", "/eventsconnect/events");
		cy.get(".grid a:first").within(() => {
			cy.get("div").contains("Test Event");
			cy.get("div").contains(
				"Test Event Short Introduction to test the UI"
			);
			cy.get(".event-image")
		...
		cy.url().should("include", "/eventsconnect/events/test-event");
		cy.get("div").contains("Test Event");
		cy.get("div").contains("Test Event Short Introduction to test the UI");
